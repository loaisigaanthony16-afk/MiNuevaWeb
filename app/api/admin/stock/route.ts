...
import { pushRestock } from "@/lib/push-server";
...
export async function PUT(request: Request) {
  ...
  try {
    const before = (await listStock())[id];
    ...
    const after = (await listStock())[id];
    // Estaba agotado y volvió: aviso a quienes pidieron que se les avise.
    const wasOut = before !== undefined && before <= 0;
    const isIn = after === undefined || after > 0;
    if (wasOut && isIn) await pushRestock(id, getProduct(id)?.name ?? "Tu sabor");
